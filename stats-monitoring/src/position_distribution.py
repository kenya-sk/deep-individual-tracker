...
def load_current_coordinate(cfg: DictConfig, frame_num: int) -> pd.DataFrame:
    ...
    coordinate_path = os.path.join(cfg['path']['coordinate_directory'], f'{frame_num}.csv')
...
def load_past_coordinate(cfg: DictConfig) -> pd.DataFrame:
    ...
    if os.path.isdir(cfg["path"]["past_coordinate_directory"]):
        ...
        print(f'[ERROR] Not Exist Directory: {cfg["path"]["past_coordinate_directory"]}')
        sys.exit(1)
...
def set_histogram(cfg: DictConfig, current_coordinate_df: pd.DataFrame, past_coordinate_df: pd.DataFrame,
                  x_ax: plt.axis, y_ax: plt.axis) -> NoReturn:
    ...
    sns.distplot(
        past_coordinate_df['x'], bins=x_bins, color='#ff6347', axlabel=False,
        label='Past Distribution \n(1 hour)', hist=False, kde=True, rug=False, ax=x_ax)

    # current X-axis distribution
    sns.distplot(
        current_coordinate_df['x'], bins=x_bins, color='#0000cd', axlabel=False,
        label='Current Distribution', hist=False, kde=True, rug=False, ax=x_ax)
    ...
    # past 1 hour distribution
    sns.distplot(
        past_coordinate_df['y'], bins=y_bins, color='#ff6347', axlabel=False,
        label='Past \nDistribution \n(1 hour)', hist=False, kde=True, rug=False, vertical=True, ax=y_ax)

    # current Y-axis distribution
    sns.distplot(
        current_coordinate_df['y'], bins=y_bins, color='#0000cd', axlabel=False,
        label='Current \nDistribution', hist=False, kde=True, rug=False, vertical=True, ax=y_ax)
    ...
    y_ax.invert_xaxis()
    y_ax.invert_yaxis()
    y_ax.tick_params(labelbottom=False,
                     labelleft=False,
                     labelright=False,
                     labeltop=False,
                     bottom=False,
