...
cv::Mat padding(cv::Mat& frame, int padding_size) {
    /**
     * input:
    ...
    **/
    ...
    cv::Mat pad_frame = cv::Mat::zeros(cv::Size(pad_width, pad_height), CV_8UC3);
...
std::vector<int> local_match_template(cv::Mat& prev_frame, cv::Mat& current_frame, std::vector<int>& feature_vec, int template_size, int search_width, float matching_thresh) {
    /**
    ...
    **/
    ...
    cv::Mat padding_prev_frame = padding(prev_frame, padding_size);
    cv::Mat padding_current_frame = padding(current_frame, padding_size);
    ...
    cv::Mat template_img = padding_prev_frame(template_region);
    ...
    cv::Mat local_img = padding_current_frame(local_region);
...
void tracking(string video_filepath, string cord_dircpath, string output_stats_dirc, string output_video_path) {
    ...
    **/
    ...
    if (!capture.isOpened()){
        cout << "ERROR: can not open file (output video). please check file path." << endl;
        cout << "PATH: " << video_filepath << endl;
        exit(1);
    }
    ...
    int skip = 30; // skip width of frame
    int tracking_thresh = 15;
    int cord_index = 4*skip;
    int frame_num = 0;
    cv::Mat  prev_frame, current_frame, feature_point_frame;
    cv::Mat tracking_mask = cv::Mat::zeros(cv::Size(width, height), CV_8UC3);
    
    //skip untill first codinate
    ...
    capture >> prev_frame;
    std::vector< std::vector<int> > prev_cord;
    read_csv(cord_dircpath + std::to_string(cord_index) + ".csv",prev_cord,',');
    //read_csv(cord_file_vec.at(cord_index), prev_cord, ',');
    std::vector<int> current_matching_cord;
    std::vector< std::vector<int> > current_matching_cord_vec;
    std::vector<int> movement;
    std::vector< std::vector<int> > movement_vec;
    std::vector<float> movement_norm_vec;

    // record whether matching point was found or not (True or False)
    std::vector<bool> record_matching_vec;
    record_matching_vec.resize(prev_cord.size(), true);
    std::vector< std::vector<int> > start_point_vec;
    start_point_vec = prev_cord;

    // dgree of direction (every frame)
    std::vector<int> start_point(2, 0);
    std::vector<int> end_point(2, 0);
    int degree = 0;
    std::vector<int> frame_degree_vec;
    // save degree of 1sec
    std::vector< std::vector<int> > degree_vec;
    degree_vec.reserve(total_frame+10);

    // statistic value
    float movement_mean, movement_var, movement_max;
    int window_size = (int)fps; // window size is the amount to hold statistics value
    std::deque<float> mean_window_deq(window_size-1, 0.0), var_window_deq(window_size-1, 0.0), max_window_deq(window_size-1, 0.0);
    std::vector<float> mean_vec, var_vec, max_vec;
    mean_vec.reserve(total_frame+10);
    var_vec.reserve(total_frame+10);
    max_vec.reserve(total_frame+10);
    ...
    while(true) {
        ...
        for (int idx=0;idx<prev_cord.size();++idx) {
            if (record_matching_vec.at(idx)){
                ...
                current_matching_cord = local_match_template(prev_frame, current_frame, prev_cord.at(idx), 50, 30, 0.7);
        ...
        // skip last 3sec frame (company logo)
        // fix frame_num_vec size and stats_vec size
        if (frame_num >= total_frame-3*skip){
            break;
        }
