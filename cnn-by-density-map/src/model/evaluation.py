...
from utils import eval_metrics, get_masked_index, pretty_print
    ...
    for skip in args.skip_width_lst:
        ...
        for path in tqdm(abs_path_lst):
            ...
            if centroid_arr.shape[0] == 0:
                logger.debug(
                    "file name: {0}, Not found point of centroid. Accuracy is 0.0".format(
                        file_name
                    )
                )
                ...
                accuracy, precision, recall, f_measure = eval_metrics(
                    true_pos, false_pos, false_neg, n
                )
                logger.debug(
                    "file_name: {0}, accuracy: {1}, precision: {2}, recall: {3}, f-measure: {4}".format(
...
def make_eval_parse():
    ...
    # Parameter Argument
    parser.add_argument(
        "--dist_thresh", type=int, default=15, help="threshold of detect or not"
    )
    parser.add_argument(
        "--skip_width_lst", type=list, default=[15], help="test by each skip width"
    )
